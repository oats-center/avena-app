...
<main>
	...
	<div class="flex items-center mt-30 mb-16 h-96 border border-base-300 break-before-avoid">
		...
		<div class="radial-progress mt-30" style="--value:70; --size:16rem; --thickness: 2px;">
			CPU Utilization <br /> 70%
		</div>
		<div class="radial-progress mt-30" style="--value:70; --size:16rem; --thickness: 2rem;">
			Memory Utilization <br /> 80%
		</div>
	</div>
	...
	<div class="grid grid-cols-3 gap-2 mb-16">
		<div>
			{#each ActiveService as service}
				...
				{@const MemoryShare =
					Math.round((service.memory_current / service.memory_available) * 100) % 100}
				<!-- {checked = IsRunning} -->
				...
				<div class="stats shadow max-w-fit item-center">
					...
					<div class="stat w-48">
						<div class="stat-value">
							{((service.cpu_usage_n_sec / service.cpu_shares) * 100).toFixed(2)}%
						</div>
						<div class="stat-title">CPU Usage</div>
					</div>
					<div class="btn-group h-full">
						<button on:click={EndService} type="submit" class="w-full h-full btn btn-active">
							End Service
						</button>
					</div>
				</div>
	...
	<div class="grid grid-cols-3 gap-2 mb-16">
		<div>
			{#each InactiveService as Iservice}
				...
				{@const MemoryShare =
					Math.round((Iservice.memory_current / Iservice.memory_available) * 100) % 100}
				<!-- {checked = IsRunning} -->
            ...
            {/if} -->
                ...
                <div class="stat-title">Memory Usage</div>
					...
					<div class="btn-group h-full">
						<button on:click={StartService} type="submit" class="w-full h-full btn">
							Start Service
						</button>
					</div>
				</div>
